...
//        System.out.println(fileList.size());
    ...
    private void parseHTML(File f) {
        ...
        // 3. 解析出 HTML 对应的正文 (有了正文才有后续的描述，描述展示了正文的一部分)
    ...
    public String parseContent(File f) {
        ...
        // BufferedReader类以字符为单位读取，FileInputStream类以字节为单位读取
        // 先按照一个字符一个字符的方式来读取. 以 < 和 > 来控制拷贝数据的开关.
        // 手动把缓冲区设置成 1M 大小~
        //try语法：try with resource 可以省略bufferedReader.close() 自动执行关闭操作
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(f), 1024 * 1024)) {
            ...
            while (true) {
                ...
                if (isCopy) {
                    ...
                    if (c == '\n' || c == '\r') {
                        //如果没有这句代码，得到的结果里面存在很多换行，不太合适。为了更好的生成描述信息，我们要去掉空行。
                        // 为了去掉换行, 把换行替换成空格了.
                        c = ' ';
                    }
    ...
    private void enumFile(String inputPath, ArrayList<File> fileList) {
        ...
        //System.out.println(Arrays.toString(files));  //api目录底下的文件
        ...
        for (File f : files) {
            ...
            if (f.isDirectory()) {
                ...
                //我们要搜索的是Java API文档，不相关的文件暂不在搜索范围
                //判断当前文件是否以.html结尾
